...
namespace RurouniJones.Telemachus.Core.Collectors
{
    public class UnitCollector : ICollector
    {
        ...
        public UnitCollector(ILogger<UnitCollector> logger)
        {
            _logger = logger;
        ...
        public async Task MonitorAsync(ICollector.CollectorConfig config)
        {
            ...
            _meter.CreateObservableGauge("units_per_server_gauge",
                () => { return GetUnits(serverShortName, sessionId); },
        ...
        private List<Measurement<int>> GetUnits(string serverShortName, long sessionId)
        {
